...
import { sendContextMessage, sendMessage } from './services';
...
const chatSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      builder
      .addCase(sendContextMessage.pending, (state) => {
        console.log('Pending state:', state);
        state.isLoading = true;
        state.error = null;
      })
      .addCase(sendContextMessage.fulfilled, (state, action) => {
        console.log('Fulfilled action:', action);
        state.isLoading = false;
        const userMessage = {
          type: 'user',
      ...
      .addCase(sendContextMessage.rejected, (state, action) => {
        console.log('Rejected action:', action);
        state.isLoading = false;
        state.error = action.error.message;
        const userMessage = {
